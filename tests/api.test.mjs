...
test("migrate existing uploads, enforce the fixed password, and save multiple files with points in one project",async()=>{
 ...
 const photoContext={params:Promise.resolve({id:ids[1],photo:"p0.jpg"})};
 assert.equal((await photo.DELETE(request("/api/trips/"+ids[1]+"/photos/p0.jpg",undefined,"DELETE"),photoContext)).status,403);
 assert.equal((await projects.DELETE(request("/api/projects?id="+project.id,undefined,"DELETE"))).status,403);
 ...
 const edited=await jsonOk(await photo.DELETE(request("/api/trips/"+ids[1]+"/photos/p0.jpg",undefined,"DELETE"),photoContext));
 assert.equal(edited.photoCount,0);assert.equal(edited.points.length,2);assert.equal(edited.cover,undefined);
 assert.equal(await env.BUCKET.head("trips/"+ids[1]+"/photos/p0.jpg"),null);
...
test("the Worker routes browser CORS preflights and API calls from GitHub Pages",async()=>{
 ...
 assert.equal(listing.status,200);assert.ok(Array.isArray((await listing.json()).trips));
});
