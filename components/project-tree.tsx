...
type Props={groups:ProjectGroup[];visible:Set<string>;selectedId:string|null;loadingIds:Set<string>;onToggle:(id:string,show:boolean)=>void;onToggleProject:(files:TripSummary[],show:boolean)=>void;onSelect:(id:string)=>void;onAdd:(projectId:string)=>void;onEdit:(file:TripSummary)=>void;canDelete:boolean;onDeleteFile:(id:string)=>void;onDeleteProject:(id:string)=>void;canEdit:boolean};
export default function ProjectTree({groups,visible,selectedId,loadingIds,onToggle,onToggleProject,onSelect,onAdd,onEdit,canEdit,canDelete,onDeleteFile,onDeleteProject}:Props){
 ...
 return <div className="project-tree">{groups.map(group=>{
  ...
  return <section className="project-group" key={group.id} aria-label={group.name}>
   ...
   {canDelete&&!group.unassigned&&<button className="delete-project-button" onClick={()=>onDeleteProject(group.id)} aria-label={"Delete project "+group.name}><Trash2 size={14}/>Delete project</button>}
   {!closed&&<div className="project-files">{!group.files.length?<button className="empty-project" onClick={()=>onAdd(group.id)}><FolderOpen size={18}/><span>Add your first KMZ or KML</span><Plus size={15}/></button>:group.files.map(file=><div className={"trip-row "+(selectedId===file.id?"selected":"")} key={file.id}>
    ...
    <div className="file-row-actions">{loadingIds.has(file.id)?<LoaderCircle size={15} className="spin"/>:<span className="route-color" style={{background:file.color}}/>}{canEdit&&<button className="file-edit-button" aria-label={"Edit "+file.name} onClick={()=>onEdit(file)}>Edit</button>}{canDelete&&<button className="icon-button" title="Delete file" aria-label={"Delete file "+file.name} onClick={()=>onDeleteFile(file.id)}><Trash2 size={15}/></button>}</div>
   </div>)}</div>}
