...
export default function FieldAtlas() {
 ...
 return <SidebarProvider style={{"--sidebar-width":"224px"} as React.CSSProperties} className="atlas-app">
  ...
  <AlertDialog open={!!confirm} onOpenChange={v=>{if(!v&&!removing)setConfirm(null);}}><AlertDialogContent className="atlas-dialog"><AlertDialogHeader><AlertDialogTitle>{confirm==="drafts"?"Clear unfinished uploads?":"Delete this file?"}</AlertDialogTitle><AlertDialogDescription>{confirm==="drafts"?"This removes incomplete uploads, including any currently in progress. Completed files are kept.":"The saved route, photographs, and original file will be permanently removed from this website. Keep your own copy before deleting."}</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel disabled={removing}>Cancel</AlertDialogCancel><AlertDialogAction disabled={removing} className="wine-button" onClick={e=>{e.preventDefault();void removeConfirmed();}}>{removing?"Deleting…":confirm==="drafts"?"Clear uploads":"Delete file"}</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog>
